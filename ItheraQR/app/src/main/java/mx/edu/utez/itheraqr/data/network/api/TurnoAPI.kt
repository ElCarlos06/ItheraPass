    ...
    fun getTurnos(
        ...
        val listener = Response.Listener<JSONArray> { response ->
            ...
            for (i in 0 until response.length()) {
                ...
                // Limpiar el código del turno (quitar "A-" si existe para compatibilidad con turnos antiguos)
                val codigoRaw = objeto.optString("codigoTurno", "0")
                val codigoLimpio = if (codigoRaw.startsWith("A-")) {
                    codigoRaw.substringAfter("-")
                } else {
                    codigoRaw
                }
